...
Operator가 Task를 생성하고 상태(todo → in_progress → done)를 변경할 수 있어야 한다. 변경 시 actor, from, to, timestamp가 감사 로그로 남아 조회 가능해야 한다. 그리고 모바일 앱이랑 슬랙 봇도.
