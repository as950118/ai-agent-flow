...
Operator가 Task를 생성하고 상태(todo → in_progress → done)를 변경할 수 있어야 한다. 변경 시 actor, from, to, timestamp가 감사 로그로 남아 조회 가능해야 한다. 기존 ADR과 충돌 여부를 decision-memory에서 확인.
